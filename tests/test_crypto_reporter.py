...
def test_validate_jwk_rejects_key_ops_forbidding_wrap():
    """Keys that explicitly forbid wrapKey and encrypt should be rejected."""
    jwk = {"kty": "RSA", "n": "abc123", "e": "AQAB", "key_ops": ["verify"]}
    assert validate_jwk_public_key(jwk) is False


def test_validate_jwk_accepts_wrapkey_ops():
    """Keys with wrapKey in key_ops should be accepted."""
    jwk = {"kty": "RSA", "n": "abc123", "e": "AQAB", "key_ops": ["wrapKey"]}
    assert validate_jwk_public_key(jwk) is True


def test_validate_jwk_accepts_encrypt_ops():
    """Keys with encrypt in key_ops should be accepted."""
    jwk = {"kty": "RSA", "n": "abc123", "e": "AQAB", "key_ops": ["encrypt"]}
    assert validate_jwk_public_key(jwk) is True


def test_validate_jwk_valid_no_key_ops():
    """Keys without key_ops field should be accepted."""
